...
const GroupController = {
    ...
    async GetGroupById(req, res, next) {
        ...
        try {
            ...
            const found = await GroupService.GetGroupById(id);
    ...
    async CreateOrUpdate(req, res, next) {
        ...
        if (currentGroup) {
            ...
            try {
                ...
                if (isNameUsed) {
                    return res.status(400).send('Group with such name exists already');
                }
    ...
    async AddUsers(req, res, next) {
        ...
        if (notValidId(groupId)) {
            return res.status(400).send('Group id is not valid');
        }

        if (userIds.some(notValidId)) {
            return res.status(400).send('Some users id are not valid');
        }
